...
# Transect lines
yus_line = LineString([(float(r["lon"]), float(r["lat"])) for r in yus_points])
mtw_line = LineString([(float(r["lon"]), float(r["lat"])) for r in mtwilhelm_points])
lines_gdf = gpd.GeoDataFrame(
    {"transect": ["YUS", "Mt Wilhelm"], "geometry": [yus_line, mtw_line]}, crs="EPSG:4326"
)
...
def build_static_map(
    ...
    # Derive map extent from data with 0.5° padding
    lons = [p["lon"] for p in yus_points + mtwilhelm_points]
    lats = [p["lat"] for p in yus_points + mtwilhelm_points]
    transformer = pyproj.Transformer.from_crs("EPSG:4326", "EPSG:3857", always_xy=True)
    x_min, y_min = transformer.transform(min(lons) - 0.5, min(lats) - 0.5)
    x_max, y_max = transformer.transform(max(lons) + 0.5, max(lats) + 0.5)
...
def build_interactive_map(
    ...
    m = folium.Map(
        location=[center_lat, center_lon], zoom_start=7, tiles="OpenTopoMap", attr="OpenTopoMap"
    )
...
if __name__ == "__main__":
    ...
    build_static_map(
        gdf, lines_gdf, yus_points, mtwilhelm_points, colors, DOCS_DIR / "transect_map.png"
    )
    build_interactive_map(df, yus_points, mtwilhelm_points, colors, DOCS_DIR / "transect_map.html")
