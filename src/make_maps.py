...
def build_static_map(gdf, lines_gdf, yus_points, mtwilhelm_points, colors, out_png):
    ...
    try:
        ...
        ctx.add_basemap(ax_map, crs="EPSG:3857",
                        source=ctx.providers.OpenStreetMap.Mapnik, zoom=9, alpha=0.9)
    ...
    ax_map.legend(handles=handles, loc="lower left", fontsize=8.5,
                  framealpha=0.92, edgecolor="#aaaaaa", fancybox=True)
...
def build_interactive_map(df, yus_points, mtwilhelm_points, colors, out_html):
...
if __name__ == "__main__":
    ...
    build_static_map(gdf, lines_gdf, yus_points, mtwilhelm_points, colors,
                     DOCS_DIR / "transect_map.png")
    build_interactive_map(df, yus_points, mtwilhelm_points, colors,
                          DOCS_DIR / "transect_map.html")
