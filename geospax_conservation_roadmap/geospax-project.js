...
(function (root) {
  ...
  GSX.getProjectMeta = function () {
    if (!root._gsxProject) {
      root._gsxProject = {
        ...
        studentName: '', studentId: '', species: '', title: ''
      };
    }
    ...
    host.innerHTML =
      ...
      rows.map(function (r) {
        ...
        return '<tr>' + cols.map(function (c) {
          ...
          return '<td' + (flag ? ' style="background:#fed7d7"' : '') + '>' +
                 (v || (flag ? '— required —' : '')) + '</td>';
        }).join('') + '</tr>';
      }).join('') +
      '</tbody></table>' +
  ...
  GSX.uiSaveProject = function () {
    var meta = GSX.getProjectMeta();
    ['studentName', 'studentId', 'species', 'title'].forEach(function (k) {
      var el = document.getElementById('gsx-proj-' + k);
      if (el) meta[k] = el.value;
    });
    var json = GSX.projectToJSON(true);
    var name = (meta.studentId || 'project') + '_' +
               (meta.species || 'geospax').replace(/[^\w-]+/g, '_') + '.gspx';
  ...
  GSX.uiLoadProject = function (file) {
    ...
    reader.onload = function (e) {
      ...
      root._gsxProject = {
        ...
        studentName: (res.project.student || {}).name || '',
        studentId: (res.project.student || {}).id || '',
        species: (res.project.student || {}).species || '',
        title: (res.project.student || {}).title || ''
      };
