...
async def main():
    async with async_playwright() as p:
        ...
        report(
            "haversineKm returns distance",
            float(math_results["haversineKm"]) > 0,
            math_results["haversineKm"],
        )
        report(
            "hexToRgba converts",
            "rgba(255,0,0,0.5)" in math_results["hexToRgba"],
            math_results["hexToRgba"],
        )
        ...
        report(
            "hotspotColor returns color",
            "#" in math_results["hotspotColor"] or "rgb" in math_results["hotspotColor"],
            math_results["hotspotColor"],
        )
        report(
            "suitabilityColor returns color",
            "#" in math_results["suitabilityColor"] or "rgb" in math_results["suitabilityColor"],
            math_results["suitabilityColor"],
        )
        ...
        report(
            "detectGeomType returns type",
            geom
            in ("Point", "LineString", "Polygon", "MultiPoint", "MultiLineString", "MultiPolygon"),
            str(geom),
        )
        ...
        report(
            "getLayerAttributes returns array",
            isinstance(attrs, list) or "error" not in str(attrs),
            str(attrs)[:80],
        )
        ...
        report(
            "removeUploadedLayer removes layer",
            removed == "ok" and n_after == n_before - 1,
            f"{n_before}->{n_after}",
        )
        ...
        report(
            "computeClassificationBreaks returns breaks",
            "error" not in breaks and "[" in breaks,
            breaks[:80],
        )
        ...
        report(
            "buildClassifiedGradientCss returns CSS",
            "ok:" in grad or "gradient" in grad,
            str(grad)[:80],
        )
        ...
        cfh = await page.evaluate(
            """() => { try { clearFeatureHighlight(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("clearFeatureHighlight no crash", cfh == "ok", cfh)

        # Test clearFeatureSelection
        cfs = await page.evaluate(
            """() => { try { clearFeatureSelection(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        ...
        # Test zoomToFeature
        ztf = await page.evaluate(
            """() => { try { zoomToFeature(uploadedLayers[0], 0); return 'ok'; } catch(e) { return e.message; } }"""
        )
        ...
        tlp = await page.evaluate(
            """() => { try { toggleLeftPanel(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("toggleLeftPanel no crash", tlp == "ok", tlp)
        await page.evaluate("toggleLeftPanel()")  # toggle back

        # Test toggleRightPanel
        trp = await page.evaluate(
            """() => { try { toggleRightPanel(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        ...
        tt = await page.evaluate(
            """() => { try { toggleTheme(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("toggleTheme no crash", tt == "ok", tt)
        await page.evaluate("toggleTheme()")  # toggle back

        # Test switchLpTab
        slt = await page.evaluate(
            """() => { try { switchLpTab('layers'); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("switchLpTab no crash", slt == "ok", slt)

        # Test closeLayerContextMenu
        clcm = await page.evaluate(
            """() => { try { closeLayerContextMenu(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("closeLayerContextMenu no crash", clcm == "ok", clcm)

        # Test collapseAllSectionsByDefault
        casb = await page.evaluate(
            """() => { try { collapseAllSectionsByDefault(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        ...
        for fn in [
            ...
            "refreshAnalysisAttrSelect",
            "refreshAttrLayerSelect",
            "refreshExportLayerSelect",
            "refreshGSXRasterLayerSelect",
            "refreshGSXSDMLayers",
            "refreshGSXWLCConstraint",
            "refreshHotspotLayerRows",
            "refreshSymbologyLayerSelect",
            "refreshMapLegend",
        ]:
            r = await page.evaluate(
                f"""() => {{ try {{ {fn}(); return 'ok'; }} catch(e) {{ return e.message; }} }}"""
            )
            report(f"{fn} no crash", r == "ok", r)

        # Test updateStats
        us = await page.evaluate(
            """() => { try { updateStats(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("updateStats no crash", us == "ok", us)

        # Test showResult
        sr = await page.evaluate(
            """() => { try { showResult('test', 'Test result'); return 'ok'; } catch(e) { return e.message; } }"""
        )
        ...
        lat = await page.evaluate(
            """() => { try { loadAttrTable(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("loadAttrTable no crash", lat == "ok", lat)

        # Test sortAttrTable
        sat = await page.evaluate(
            """() => { try { sortAttrTable(0, 'asc'); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("sortAttrTable no crash", sat == "ok", sat)

        # Test filterAttrTable
        fat = await page.evaluate(
            """() => { try { filterAttrTable('test'); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("filterAttrTable no crash", fat == "ok", fat)

        # Test updateFeatureCount
        ufc = await page.evaluate(
            """() => { try { updateFeatureCount(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        ...
        st = await page.evaluate(
            """() => { try { showToast('test', 'success'); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("showToast no crash", st == "ok", st)

        # Test showProcessing / hideProcessing
        sp = await page.evaluate(
            """() => { try { showProcessing('test'); return 'ok'; } catch(e) { return e.message; } }"""
        )
        ...
        ce = await page.evaluate(
            """() => { try { closeExport(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("closeExport no crash", ce == "ok", ce)

        # Test loadGeoTIFF with null
        lgt = await page.evaluate(
            """() => { try { loadGeoTIFF(null); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report(
            "loadGeoTIFF null no crash",
            lgt == "ok" or "no" in lgt.lower() or "null" in lgt.lower(),
            lgt,
        )

        # Test populateCitationFormats
        pcf = await page.evaluate(
        ...
        rb = await page.evaluate(
            """() => { try { renderBookmarks(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("renderBookmarks no crash", rb == "ok", rb)

        # Test goToBookmark
        gtb = await page.evaluate(
            """() => { try { goToBookmark(0); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("goToBookmark no crash", gtb == "ok", gtb)

        # Test deleteBookmark
        db = await page.evaluate(
            """() => { try { deleteBookmark(0); return 'ok'; } catch(e) { return e.message; } }"""
        )
        ...
        tcs = await page.evaluate(
            """() => { try { toggleComposerSidebar(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("toggleComposerSidebar no crash", tcs == "ok", tcs)
        await page.evaluate("toggleComposerSidebar()")  # toggle back

        # Test updateOverview
        uo = await page.evaluate(
            """() => { try { updateOverview(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        report("updateOverview no crash", uo == "ok" or "no" in uo.lower(), uo)

        # Test setBasemapFromSelect
        sbs = await page.evaluate(
            """() => { try { setBasemapFromSelect(); return 'ok'; } catch(e) { return e.message; } }"""
        )
        ...
        ils = await page.evaluate(
            """() => { try { initLegendState(uploadedLayers[0]); return 'ok'; } catch(e) { return e.message; } }"""
        )
        ...
        ulb = await page.evaluate(
            """() => { try { updateLabelBtnState(); return 'ok'; } catch(e) { return e.message; } }"""
        )
