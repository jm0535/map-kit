...
async def main():
    async with async_playwright() as p:
        ...
        # Mock the download to capture the .gspx content. uiSaveProject opens
        # a confirmation dialog; hide the File System Access picker so the
        # confirm falls back to a standard download we can intercept.
        gspx_data = await page.evaluate("""async () => {
            ...
            // Call save (dialog appears), then confirm it
            try {
                delete window.showSaveFilePicker;
                GSX.uiSaveProject();
                await new Promise(r => setTimeout(r, 300));
                const btn = document.querySelector('.attr-form-overlay .btn-ok');
                if (btn) btn.click();
            } catch(e) { return 'error: ' + e.message; }
            finally {
                URL.createObjectURL = origCreate;
                URL.revokeObjectURL = origRevoke;
                if (origPicker === undefined) delete window.showSaveFilePicker;
                else window.showSaveFilePicker = origPicker;
            }
            // Wait for the async download to produce the blob
            for (let i = 0; i < 20 && !captured; i++) await new Promise(r => setTimeout(r, 200));
            return captured;
        }""")
        report(
            "Save project produces JSON",
            gspx_data and gspx_data.startswith("{"),
            f"len={len(gspx_data) if gspx_data else 0}",
        )

        if gspx_data and gspx_data.startswith("{"):
            proj = json.loads(gspx_data)
            report(
                "Saved project has layers",
                "layers" in proj and len(proj.get("layers", [])) > 0,
                f"layers={len(proj.get('layers', []))}",
            )
            report(
                "Saved project has metadata",
                "author" in proj or "authorName" in proj,
                str(list(proj.keys())[:8]),
            )

            # ── Load project ──
            # Write the .gspx to a temp file and load it
            gspx_path = os.path.join(tempfile.gettempdir(), "gsx_test_save.gspx")
        ...
        report(
            ...
            autosave is not None,
            f"len={len(autosave) if autosave else 0}",
        )

        if autosave:
            auto_obj = json.loads(autosave)
            report(
                "Autosave has layers", "layers" in auto_obj and len(auto_obj.get("layers", [])) > 0
            )
